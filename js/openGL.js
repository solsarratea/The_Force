...
function createGlContext() {
  ...
  $.when(
    $.ajax({ url: "shaders/draw.vert", dataType: "text" }),
    ...
    if (screenProgram !== null) gl.deleteProgram(screenProgram);
...
function newShader(vs, shaderCode) {
  var res = createShader(
    vs,
    mHeader + mInputsStr + mOSCStr + mMIDIStr + shaderCode
  ); //, true);
  ...
  if (mProgram !== null) gl.deleteProgram(mProgram);
  ...
  ch5 = gl.getUniformLocation(mProgram, "drawing");
...
function createInputStr() {
  ...
  for (var i = 0; i < mInputs.length; i++) {
    ...
    else mInputsStr += "uniform sampler2D channel" + i + ";\n";
  }
}
...
function createDrawingCanvasTexture(ctx) {
  if (drawingCanvasCanvas == null) {
    console.log("drawingCanvasCanvas is null");
    return;
  }
...
function paint() {
  ...
  if (l4 !== null)
    gl.uniform4f(l4, mMousePosX, mMousePosY, mMouseClickX, mMouseClickY);
  if (l7 !== null)
    gl.uniform4f(
      l7,
      d.getFullYear(),
  ...
  if (ch5 !== null) gl.uniform1i(ch5, 5); //drawing
  ...
  if (mSound && bandsOn && mAudioContext !== null) {
    ...
    if (bandsTimeU !== null) {
      //this is for per fft band time elapsed events
      if (mSound.low > 0.7) elapsedBandPeaks[0] = 0.0;
      else elapsedBandPeaks[0] += meter.duration * 0.001;

      if (mSound.mid > 0.7) elapsedBandPeaks[1] = 0.0;
      else elapsedBandPeaks[1] += meter.duration * 0.001;

      if (mSound.upper > 0.7) elapsedBandPeaks[2] = 0.0;
      else elapsedBandPeaks[2] += meter.duration * 0.001;

      if (mSound.high > 0.7) elapsedBandPeaks[3] = 0.0;
      else elapsedBandPeaks[3] += meter.duration * 0.001;
  ...
  for (var i = 0; i < mInputs.length; i++) {
    ...
    if (inp === null) {
      ...
      if (inp.loaded === false) gl.bindTexture(gl.TEXTURE_2D, null);
      else {
  ...
  if (numScreens == 1) {
    ...
    gl.uniform4f(screenBlendU, 0.0, 0.001, 1.0, 0.001);
    ...
    if (testingImage) gl.bindTexture(gl.TEXTURE_2D, testTexture);
    else gl.bindTexture(gl.TEXTURE_2D, fbos[pingPong].texture);
